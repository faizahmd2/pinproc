...
func newCaptureCmd() *cobra.Command {
	...
	cmd := &cobra.Command{Use: "capture [host]", Short: "capture one investigation for offline replay", Args: cobra.MaximumNArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		...
		base := out
		if base == "" {
			base, _ = config.ResolveOutputDirectory(cfg.Output.Directory)
		}
		...
		if budgetName == "normal" {
			b, _ = budget(cfg.Engine.Budget)
		}
		dec, err := makeDecisionProvider(cfg, noAI)
		if err != nil {
			return err
		}
		id := fmt.Sprintf("inv-%d", time.Now().UnixNano())
		captureDir := filepath.Join(base, id)
		...
		inv, err := eng.Run(context.Background(), engine.Request{ID: id, Host: host, Trigger: trigger, Hint: hint, Dimension: contract.Dimension(dim)})
		if err != nil {
			return err
		}
	...
	cmd.Flags().StringVar(&out, "out", "", "report/capture directory")
