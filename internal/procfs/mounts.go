...
func ParseMountInfo(b []byte) ([]MountInfo, error) {
	...
	for _, line := range strings.Split(string(b), "\n") {
		...
		sep := -1
		for i, f := range fields { if f == "-" { sep = i; break } }
		if sep < 6 || len(fields) <= sep+3 { continue }
		id, e1 := strconv.Atoi(fields[0]); parent, e2 := strconv.Atoi(fields[1])
		dev := strings.SplitN(fields[2], ":", 2)
		if e1 != nil || e2 != nil || len(dev) != 2 { continue }
		major, e3 := strconv.ParseUint(dev[0],10,64); minor, e4 := strconv.ParseUint(dev[1],10,64)
		if e3 != nil || e4 != nil { continue }
	...
	if len(out)==0 { return nil, errors.New("no mount records") }
	return out,nil
}
