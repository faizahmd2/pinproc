...
func readTailBounded(ctx context.Context, path string, limit int64, timeout time.Duration) ([]byte, error) {
	...
	case <-ctx.Done():
		return nil, ctx.Err()
	case <-timer.C:
		return nil, fmt.Errorf("log read timed out after %s", timeout)
	}
}
...
func bestLine(ctx context.Context, paths []string) (string, string, int) {
	for _, path := range paths {
		...
		for sc.Scan() {
			...
			if res.Cluster == nil || !severityRe.MatchString(line) {
				continue
			}
			key := res.Cluster.GetTemplate()
			p := picks[key]
			if p == nil {
				p = &linePick{template: key}
				picks[key] = p
			}
			p.count = res.Cluster.Size
			p.line = line
			p.order = order
		}
		var best *linePick
		for _, p := range picks {
			if best == nil || p.count > best.count || (p.count == best.count && p.order > best.order) {
				best = p
			}
		}
		if best != nil {
			return path, truncate(best.line, 200), best.count
		}
		if lastNonEmpty != "" {
			return path, truncate(lastNonEmpty, 200), 1
		}
	}
	return "", "", 0
}
