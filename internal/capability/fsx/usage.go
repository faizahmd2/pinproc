...
func parseFilesystem(in spec.ParseInput) (contract.Evidence, error) {
	...
	if err != nil { return contract.Evidence{}, err }
	rows := make([]MountFacts, 0, len(mounts))
	seen := map[string]bool{}
	for _, m := range mounts {
		if excluded(m.FSType) { continue }
		device := fmt.Sprintf("%d:%d", m.Major, m.Minor)
		key := device
		if m.FSType == "tmpfs" { key = "tmpfs:" + device }
		if seen[key] { continue }
		seen[key] = true
		var st syscall.Statfs_t
		if err := syscall.Statfs(m.MountPoint, &st); err != nil { continue }
		if st.Blocks == 0 { continue }
		used := float64(st.Blocks-st.Bfree) / float64(st.Blocks) * 100
		inode := 0.0
		if st.Files > 0 { inode = float64(st.Files-st.Ffree) / float64(st.Files) * 100 }
...
func parseUsage(in spec.ParseInput) (contract.Evidence, error) {
	...
	if mount == "" { return contract.Evidence{}, fmt.Errorf("mount scope is empty") }

	deadline := time.Now().Add(3 * time.Second)
	entries := 0
	dirSizes := map[string]uint64{}
	type fileRow struct { Path string; Size uint64 }
	var files []fileRow
	...
	err := filepath.WalkDir(mount, func(path string, d fs.DirEntry, walkErr error) error {
		...
		entries++
		if entries > 20000 { return filepath.SkipAll }
		if entries%128 == 0 && time.Now().After(deadline) { return filepath.SkipAll }
		if depth(path) > 6 {
			if d.IsDir() { return filepath.SkipDir }
			return nil
		}
		info, ie := d.Info()
		if ie != nil { return nil }
		size := uint64(0)
		...
		var parts []string
		if rel != "" { parts = strings.Split(rel, "/") }
		dirSizes[mount] += size
	...
	if err != nil && err != filepath.SkipAll { return contract.Evidence{}, err }

	sort.Slice(files, func(i, j int) bool { return files[i].Size > files[j].Size })
	if len(files) > 10 { files = files[:10] }
	type dirRow struct { Path string; Size uint64 }
	dirs := make([]dirRow, 0, len(dirSizes))
	for p, size := range dirSizes { dirs = append(dirs, dirRow{Path: p, Size: size}) }
