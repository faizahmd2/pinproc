...
func (s *RecordingSource) Facts(ctx context.Context) (contract.Facts, error) {
	...
	if err == nil {
		...
		if b, e := json.MarshalIndent(f, "", "  "); e == nil {
			_ = os.WriteFile(filepath.Join(s.Dir, "meta.json"), append(b, '\n'), 0644)
		}
	}
	return f, err
}
func (s *RecordingSource) Snapshot(ctx context.Context, reads []Read) (Snapshot, error) {
	...
	if e := s.saveSnapshot(x); err == nil && e != nil {
		err = e
	}
	return x, err
}
func (s *RecordingSource) Sample(ctx context.Context, reads []Read, w time.Duration) (Sample, error) {
	...
	if e := s.saveSnapshot(x.T0); err == nil && e != nil {
		err = e
	}
	if e := s.saveSnapshot(x.T1); err == nil && e != nil {
		err = e
	}
	return x, err
}
...
func (s *RecordingSource) saveSnapshot(snap Snapshot) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	root := filepath.Join(s.Dir, fmt.Sprintf("snapshot-%03d", s.seq))
	s.seq++
	...
	idx := struct {
		...
		Reads []item    `json:"reads"`
	}{At: snap.At, Bytes: snap.Bytes}
		...
		for _, raw := range rows {
			...
			if raw.Path != "" {
				if err := os.MkdirAll(filepath.Dir(path), 0755); err == nil {
					_ = os.WriteFile(path, raw.Data, 0644)
				}
			}
		}
	}
