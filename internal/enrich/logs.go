...
func Attach(ctx context.Context, src source.Source, hyps []contract.Hypothesis) []contract.Notice {
	...
	kmsgNeeded := false
	for _, h := range hyps { if h.Source == "rule:mem.oom_recent" { kmsgNeeded = true; break } }
	...
	for i := range hyps {
		...
		if hyps[i].Source == "rule:mem.oom_recent" && !kmsgChecked {
			...
			if src != nil {
				s, err := src.Snapshot(ctx, []source.Read{{Key:"kmsg",Path:"/dev/kmsg",Kind:source.ReadKmsg,MaxBytes:64<<10,Optional:true}})
				if err == nil && len(s.Reads["kmsg"]) > 0 {
					raw:=s.Reads["kmsg"][0]; kmsgLines=string(raw.Data)
					if raw.Err!=nil&&len(kmsgLines)==0 { notices=append(notices,contract.Notice{Capability:"logs.kernel",Message:"kernel log unavailable — "+raw.Err.Error(),Count:1}) }
				}
			}
			if path,line:=matchOOM(kmsgLines,hyps[i].Entity.ID);path!=""{hyps[i].LogContext=&contract.LogContext{Path:path,Line:line}}
		}
	}
	return notices
}
...
func bestLine(ctx context.Context, paths []string) (string,string,int) {
	for _, path := range paths {
		...
		info,err:=f.Stat();if err!=nil{_ = f.Close();continue}
		const tail=32<<10
		start:=info.Size()-tail;if start<0{start=0}
		if _,err=f.Seek(start,io.SeekStart);err!=nil{_ = f.Close();continue}
		data,err:=io.ReadAll(io.LimitReader(f,tail));_ = f.Close(); if err!=nil{continue}
		var miner *drain3.TemplateMiner
		miner,err=drain3.New();if err!=nil{continue}
		picks:=map[string]*linePick{};order:=0;lastNonEmpty:=""
		sc:=bufio.NewScanner(strings.NewReader(string(data)))
		for sc.Scan(){line:=strings.TrimSpace(sc.Text());if line==""{continue};lastNonEmpty=line;order++;res:=miner.AddLogMessage(line);if res.Cluster==nil||!severityRe.MatchString(line){continue};key:=res.Cluster.GetTemplate();p:=picks[key];if p==nil{p=&linePick{template:key};picks[key]=p};p.count=res.Cluster.Size;p.line=line;p.order=order}
		var best *linePick
...
func matchOOM(data, entityID string)(string,string){
	pid:=strings.TrimPrefix(entityID,"pid:");if _,err:=strconv.Atoi(pid);err!=nil{return "",""}
	for _,line:=range strings.Split(data,"\n"){m:=oomRe.FindStringSubmatch(line);if len(m)>1&&m[1]==pid{return "/dev/kmsg",truncate(line,200)}}
	return "",""
}
